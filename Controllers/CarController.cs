    ...
    public class CarController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("BrandId,ModelId,Year,Mileage,History,UserID,Registration,Status,Colour")] Car car, IFormFile carImageFile)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                // Assign a random mechanic to the car evaluation
                var mechanic = await GetRandomMechanic();
                if (mechanic != null)
                {
                    var carEvaluation = new CarEvaluation
                    {
                        CarID = car.CarID, // This assumes CarID is set after saving the car
        ...
        [Authorize(Roles = "Admin")]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("CarID,BrandId,ModelId,Year,Mileage,History,UserID,CarImage")] Car car)
        {
